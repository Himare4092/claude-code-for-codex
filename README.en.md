...
The marketplace is named `claude-code-for-codex`; the plugin itself is named `claude`. The repository includes the bundled runtime and dependencies, so end users do not need `npm install`.

If your Codex CLI does not recognize `plugin`, update to a version with plugin support. Native slash-menu visibility depends on the host. See the [official OpenAI plugin documentation](https://learn.chatgpt.com/docs/plugins) for host-level details.

### Install from a local checkout

```sh
git clone https://github.com/Himare4092/claude-code-for-codex.git
cd claude-code-for-codex
codex plugin marketplace add .
codex plugin add claude@claude-code-for-codex
```

Use either the GitHub source or the local checkout for this marketplace. Keep a local checkout in a permanent folder if you register it. The installable plugin root is `plugins/claude`; the marketplace manifest is `.agents/plugins/marketplace.json`.
